...
function findByName(e) {
  ...
  $.getJSON("https://www.thecocktaildb.com/api/json/v1/1/search.php?s=" + name.replace(/(\s)/g, "_"), function(res) {
    if (!res.drinks) {
      $(".resultsContainer").append("<p class='retrySearch'>Sorry, we weren't able to find any drinks with that name. Check the spelling or try a different search.</p>");
    }
...
function showRecipe(e) {
  ...
  $.getJSON("https://www.thecocktaildb.com/api/json/v1/1/search.php?s=" + drink, function(res) {
    ...
    var measureIngredient = ingredientArr.map((d, i) => `<li class="ingredientItem">${measureArr[i]} of ${d}</li>`);
...
function currentPage(page, size) {
  page = page || 0;
  size = size || 10;
}
